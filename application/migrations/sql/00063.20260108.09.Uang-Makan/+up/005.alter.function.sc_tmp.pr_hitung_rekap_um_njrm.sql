...
BEGIN
    ...
    FOR vr_nik in select trim(nik) from sc_mst.karyawan where tglkeluarkerja is null and kdcabang=vr_kdcabang
        LOOP
            vr_nominal_um:=case
                               ...
                               end as nominal from sc_mst.karyawan a
                                   left outer join sc_mst.uangmakan b on a.lvl_jabatan=b.kdlvl AND b.branch = case when a.idbu NOT IN ('A','B') THEN 'SBYNSA' ELSE 'NJRBJM' END
                                          ...
                                          WHEN x.rencanacallplan > 0 THEN
                                              CASE
                                                  WHEN x.realisasicallplan > 0 THEN
                                                      CASE
                                                          WHEN x.nodok IS NOT NULL AND type_ijin = 'DN' AND kendaraan = 'PRIBADI' THEN y.nominal
        ...
        sewa_kendaraan = CASE
                             WHEN x.tsewa = 'T' THEN
                                 case
                                     when x.checkin is not null THEN
                                         CASE
                                             WHEN x.nodok is not null AND type_ijin = 'DN' AND kendaraan = 'PRIBADI' THEN z.nominal
    ...
    FROM (
             ...
             FROM sc_trx.uangmakan a
                      ...
                      LEFT OUTER JOIN sc_mst.option oo ON oo.kdoption = 'B:U:E:D' AND b.idbu = any(string_to_array(oo.value1,','))
                      LEFT JOIN LATERAL (
                     ...
                     END
                      ...
                      LEFT JOIN LATERAL (
                         ...
                         WHERE xa.kdlvl = b.lvl_jabatan
                           AND xa.branch = case when b.idbu NOT IN ('A','B') THEN 'SBYNSA' ELSE 'NJRBJM' END
                 ) g ON TRUE
